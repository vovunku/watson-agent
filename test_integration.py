...
class AuditAgentTester:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                if job_id and data.get("status") in ["queued", "succeeded"]:
                    print(f"✅ Job created successfully: {job_id} (status: {data.get('status')})")
                    return job_id
                else:
                    print(f"❌ Job creation failed: {data}")
                    return None
            else:
                print(f"❌ Job creation failed with status {response.status_code}: {response.text}")
                return None
        except Exception as e:
    ...
    def test_job_report(self, job_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if content and "# Audit Report" in content and job_id in content:
                    print(f"✅ Job report retrieved successfully ({len(content)} characters)")
                    return True
                else:
                    print(f"❌ Job report content invalid")
                    return False
            else:
    ...
    def test_job_cancellation(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(f"{self.base_url}/jobs", json=payload)
            if response.status_code != 201:
                print(f"❌ Job creation for cancellation test failed: {response.status_code}")
                return False
            ...
            if cancel_response.status_code == 200:
                ...
                print(f"❌ Job cancellation failed with status {cancel_response.status_code}: {cancel_response.text}")
                return False
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run integration tests for audit agent")
    parser.add_argument("--url", default="http://localhost:8081", 
                       help="Base URL for the audit agent API")
    parser.add_argument("--timeout", type=int, default=60,
                       help="Timeout for job completion in seconds")
