...
class LLMClient:
    ...
    def __init__(self):
        ...
        logger.info(f"LLM Client initialized: dry_run={self.dry_run}, model={self.model}")
    ...
    async def analyze_code(
        ...
        """
        if self.dry_run:
            return await self._generate_dry_run_report(code, audit_profile, job_id, payload)
        else:
    ...
    async def _call_openrouter_api(
        ...
        for attempt in range(max_retries):
            try:
                ...
                delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                logger.warning(f"Timeout, retrying in {delay:.2f}s (attempt {attempt + 1})")
                await asyncio.sleep(delay)
                continue
                ...
                if attempt == max_retries - 1:
                    ...
                    raise
                delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                logger.warning(f"API call failed, retrying in {delay:.2f}s (attempt {attempt + 1}): {e}")
                await asyncio.sleep(delay)
