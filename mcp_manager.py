    ...
    async def initialize(self):
        ...
        try:
            ...
            if resp.status_code == 200 and resp.headers.get(
                ...
                logger.warning(
                    f"initialize {self.base_url} -> {resp.status_code} {resp.text[:300]}"
                )
                return
                ...
                async for chunk in s.aiter_text():
                    ...
                    for raw in buf.splitlines():
                        ...
                        try:
                            ...
                            self.initialized = True
                            logger.info(
                                f"Initialized MCP HTTP session at {self.base_url}"
                            )
                            return
                        except Exception:
                            continue
        ...
        try:
            ...
            if resp.status_code == 200 and resp.headers.get(
                ...
                if not getattr(parsed, "tools", []):
                    logger.warning(
                        f"tools/list 200 but empty from {self.base_url}: {str(data)[:300]}"
                    )
                return parsed
    ...
    def _parse_tools_json(self, data):
        ...
        try:
            ...
            if "result" in data and "tools" in data["result"]:
                ...
                logger.warning(
                    f"No tools in result from {self.base_url}: {str(data)[:300]}"
                )
        except Exception as e:
        ...
        try:
            ...
            if resp.status_code == 200 and resp.headers.get(
                ...
                if not getattr(parsed, "resources", []):
                    logger.warning(
                        f"resources/list 200 but empty from {self.base_url}: {str(data)[:300]}"
                    )
                return parsed
        ...
        try:
            ...
            async with self.http_client.stream(
                ...
                return type(
                    ...
                    {
                        "content": [
                            TextContent(type="text", text="No response from tool call")
                        ],
                        "isError": True,
                    },
                )()
        ...
        try:
            if "result" in data:
                ...
                return type(
                    ...
                    {
                        "content": [
                            TextContent(type="text", text=f"Error in response: {data}")
                        ],
                        "isError": True,
                    },
                )()
            ...
            return type(
                ...
                {
                    "content": [
                        TextContent(type="text", text=f"Parse error: {str(e)}")
                    ],
                    "isError": True,
                },
            )()
...
def convert_mcp_tool_to_langchain(
    tool_info: Dict[str, Any], session: ClientSession
) -> Any:
...
class MCPServerConnection:
    ...
    async def _connect_stdio(self):
        ...
        if self.config.token_env:
            ...
            if not token:
                logger.warning(
                    f"No token found for {self.config.name} in {self.config.token_env}"
                )
    ...
    async def _list_capabilities(self):
        ...
        try:
            ...
            if len(self.tools) == 0:
                ...
                logger.warning(
                    f"MCP server {self.config.name} connected but exposes 0 tools; marking unusable"
                )
            else:
                self.connected = True
                logger.info(
                    f"MCP server {self.config.name} usable: {len(self.tools)} tools"
                )
    ...
    async def _connect_server(self, name: str, connection: MCPServerConnection) -> bool:
        ...
        try:
            return await asyncio.wait_for(
                connection.connect(), timeout=connection.config.timeout
            )
        ...
        for tool_info in self.all_tools:
            try:
                ...
                langchain_tool = convert_mcp_tool_to_langchain(
                    tool_info, self.servers[tool_info["server"]].session
                )
                langchain_tools.append(langchain_tool)
                logger.debug(
                    f"Converted MCP tool '{tool_info['name']}' to LangChain tool"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to convert MCP tool '{tool_info['name']}' to LangChain: {e}"
                )
