...
class SimpleAuditServer:
    ...
    async def _check_access_control(self, arguments: Dict[str, Any]) -> CallToolResult:
        ...
        for i, line in enumerate(lines):
            ...
            if "function" in line_lower and (
                "public" in line_lower or "external" in line_lower
            ):
                ...
                for j in range(max(0, i - 5), i):
                    ...
                    if any(
                        mod in prev_line
                        for mod in ["onlyowner", "onlyadmin", "onlyrole"]
                    ):
    ...
    async def _get_vulnerability_info(
        self, arguments: Dict[str, Any]
    ) -> CallToolResult:
