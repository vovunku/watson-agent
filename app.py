...
from schemas import (
    ...
    HealthResponse, CancelJobResponse, ProgressInfo, MetricsInfo, JobLinks
)
...
async def health_check():
    ...
    return HealthResponse(
        ok=db_healthy,
        db="ready" if db_healthy else "error",
        version=settings.version
    )
    ...
    try:
            ...
            if existing_job:
                ...
                return CreateJobResponse(
                    ...
                    links=JobLinks(
                        ...
                        report=f"/jobs/{existing_job.job_id}/report" if existing_job.status == "succeeded" else None
                    )
                )
        ...
        return CreateJobResponse(
            ...
            links=JobLinks(
                ...
                report=f"/jobs/{job.job_id}/report"
            )
        )
...
async def get_job_status(
    ...
    try:
        ...
        return JobStatusResponse(
            ...
            links=JobLinks(
                self=f"/jobs/{job.job_id}",
                report=f"/jobs/{job.job_id}/report" if job.status == "succeeded" else None
            )
        )
